package me.sogom.springbootdeveloper.config.jwt;
...
public class TokenProvider {
    ...
    private String makeToken(Date expiry, User user) {
        ...
        return Jwts.builder()
                ...
                //내용 iss: ajufresh@gmail.com(propertise file에서 설정한 값)
                .setIssuer(jwtProperties.getIssuer())
                .setIssuedAt(now)   //내용 iat : 현재 시간
                .setExpiration(expiry)  //내용 exp : expiry 멤버 변숫값
                .setSubject(user.getEmail())    //내용 sub : user email
                .claim("id", user.getId())  //claim id : user id
                //sign : pk와 함께 Hash value를 HS256 방식으로 암호화
    ...
    public boolean validToken(String token) {
        try {
            Jwts.parser()
                    .setSigningKey(jwtProperties.getSecretKey())    //pk로 복호화
